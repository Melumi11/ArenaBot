...
class FightClass():
    ...
    async def fight(self, message):  # Fight method
        ...
        if self.mode == "17b":  # if the last roll was a 17
            if 'attack' == message.content.lower():  # attacks for 17
                if message.author.id == self.turn.id:
                    ...
                    await message.channel.send(f"You are not {self.turn.name}.")
            if 'heal' == message.content.lower():  # heals for 4d7
                if message.author.id == self.turn.id:
                    ...
                    self.mode = ""  # not 17 anymore
                    self.turn = self.p1  # change turn
                    await self.checkstuff(message)  # checks if game is over
                else:
                    await message.channel.send(f"You are not {self.turn.name}.")

        if message.content.lower() == "roll":
            if self.mode == "17b":  # if the last roll was 17 then don't roll
                await message.channel.send(f"{self.turn.name} needs to choose whether to `heal` or `attack`.")
                ...
                if self.damage != 17:
                    ...
                    await self.p1seventeen(message) # This is for if p1 got a 17, and now it's time for them to do their thing
                ...
                for i in {self.p1, self.p2}:  # 3 in a row
                    if len(i.rolls) > 2 and i.rolls[-1] == i.rolls[-2] and i.rolls[-2] == i.rolls[-3]:
                        ...
                        self.turn = i
                        return
                if self.turn == self.p2: await self.checkstuff(message)  # checks if game is over
                if self.mode != "17b": 
    ...
    async def processattack(self, message, dmg):
        if dmg == 1:  # self.damage = 0, updates stats
            ...
            self.damage = 30
        # If p2 just got a 17
        elif self.turn == self.p2 and dmg == 17:
            await self.p2seventeen(message)
        # If p1 just got a 17
        elif dmg == 17:
            await message.channel.send("You got a 17! You can do a special action after your opponent's turn.")
            self.mode = "17a"
    # These 2 functions are the starting text
    async def p2seventeen(self, message):
        self.mode = "17b"
        embedVar = discord.Embed(title=(f"{self.p2.name} got a 17!"),
    ...
    async def p1seventeen(self, message):
        if self.turn == self.p2 and self.mode == "17b" and self.p1.last == 17:
            self.turn = self.p1
            embedVar = discord.Embed(title=(f"{self.p1.name} got a 17!"),
                                    description="Would you like to `heal` or `attack`?\n(Attacking does 17 damage and healing rolls a 4d7.\nStats are only updated if you choose to heal.",
                                    color=0x00ff00)
            embedVar.set_author(name="Special Roll", icon_url=(self.turn.tag.avatar_url))
            await message.channel.send(embed=embedVar)
    ...
    async def checkstuff(self, message):
        if (self.p1.hp <= 0 and self.p2.hp <= 0) and self.p1.hp == self.p2.hp:  # draw
            embedVar = discord.Embed(
        ...
        if message.author.id == self.turn.id:
            ...
            if not winner and self.turn.id == self.p1.id: # sets p1's attack to p2.last
                ...
                if self.damage > self.p2.last: # if p2's attack is greater than p1's attack
                    if self.p2.clash < 2: self.p2.clash += 1 
                elif self.damage < self.p2.last:
                    if self.p1.clash < 2: self.p1.clash += 1
                else:
                    await message.channel.send(
            ...
            if winner and winner.clash >= 2:  # winner text and stuff, quits function
                ...
                if self.p1.last != 17: # If it isn't 17, then change hp
                ...
                await message.channel.send(embed=embedVar)
                if self.mode != "17b":
                    if self.p1.last == 1: self.p1.last = 0 # If they got a 1 then it's 0 damage
                ...
                # Handling 17 clashes
                if self.mode != "17b":
                    self.mode = "" # Set mode back if it isn't 17
                    self.turn = self.p1
                    winner = None
                    await self.checkstuff(message)  # checks if game is over
                else:
                    ...
                    if winner.id == self.p1.id:
                        winner = None
                        await self.p1seventeen(message)
                        return
                    if winner.id == self.p2.id:
                        winner = None
                        await self.p2seventeen(message)
                        return
                return
